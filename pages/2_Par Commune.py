...
# Sélection de la commune via un selectbox dans streamlit
dim_communes["commune_label"] = (
    dim_communes["nom_commune"].astype(str)
    + " (" 
    + dim_communes["nom_departement"].astype(str)
    + ")"
)

liste_communes = (
    dim_communes
    .sort_values("commune_label")["commune_label"]
    .unique()
)
...
# Récupérer le code INSEE de la commune sélectionnée = clé pour faire le filtrage des données
code_commune_selectionnee = dim_communes[
    dim_communes["commune_label"] == commune_selectionnee
]["code_commune"].values[0]
...
# Filtrer les données en fonction de la commune et de la période sélectionnées
df_conso_filtre = df_conso[
    (df_conso["code_commune"] == code_commune_selectionnee) &
    (df_conso["annee"].between(annee_debut, annee_fin))
]

df_prod_filtre = df_prod[
    (df_prod["code_commune"] == code_commune_selectionnee) &
    (df_prod["annee"].between(annee_debut, annee_fin))
]

df_pop_filtre = df_pop[
    (df_pop["code_commune"] == code_commune_selectionnee) &
...
st.write(f"### Données pour la commune de {commune_selectionnee} de {annee_debut} à {annee_fin}")
...
col1, col2 = st.columns(2)
with col1:
    st.plotly_chart(fig_conso_secteur_commune, width="stretch")
with col2:
    st.plotly_chart(fig_prod_filiere_commune, width="stretch")
...
df_evolution = (
    ...
    .merge(
        ...
        .agg({"Population": "mean"}),  # population = valeur annuelle
        on="annee"
    )
)
