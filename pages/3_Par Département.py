...
st.set_page_config(
    page_title="Bilan Energetique Par Département", 
    layout="wide",
...
# Récupérer le code INSEE du département sélectionné = clé pour faire le filtrage des données
code_departement_selectionne = dim_communes[
    dim_communes["nom_departement"] == departement_selectionne
]["code_departement"].values[0]
...
# Filtrer les données en fonction du département et de la période sélectionnées
df_conso_filtre = df_conso[
    (df_conso["code_departement"] == code_departement_selectionne) &
    df_conso["annee"].between(annee_debut, annee_fin)
]

df_prod_filtre = df_prod[
    (df_prod["code_departement"] == code_departement_selectionne) &
    df_prod["annee"].between(annee_debut, annee_fin) 
]

df_pop_filtre = df_pop[
    (df_pop["code_departement"] == code_departement_selectionne) &
...
st.write(f"### Données pour le département {departement_selectionne} de {annee_debut} à {annee_fin}")
